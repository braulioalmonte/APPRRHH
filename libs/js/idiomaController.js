...
$("#buscarIdioma").change(function(){
	var idioma = $("#buscarIdioma").val();
	var estado = $("#buscarIdioma").val();

	$("#listaIdiomas").children().remove();
	db.transaction(function(transaction){
		var sql="SELECT * FROM idiomas where idioma like '"+idioma+"%' OR estado='"+estado+"'ORDER BY id";
		transaction.executeSql(sql,undefined, function(transaction,result){
			if(result.rows.length){
